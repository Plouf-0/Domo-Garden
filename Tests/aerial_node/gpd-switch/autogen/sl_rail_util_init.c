...
#if 0U \
  || SL_RAIL_UTIL_INIT_RADIO_CONFIG_SUPPORT_GPDRAIL_ENABLE \
  || 0U
  #include "rail_config.h"
#endif

// Instance: gpdrail
static RAIL_Handle_t sl_rail_handle_gpdrail = RAIL_EFR32_HANDLE;
...
static void sl_rail_util_init_gpdrail(void)
{
#if SL_RAIL_UTIL_INIT_GPDRAIL_ENABLE
  ...
  sl_rail_handle_gpdrail = RAIL_Init(&sl_rail_config,
#if SL_RAIL_UTIL_INIT_INIT_COMPLETE_CALLBACK_GPDRAIL_ENABLE
                             &sli_rail_util_on_rf_ready
#else
                             NULL
#endif // SL_RAIL_UTIL_INIT_INIT_COMPLETE_CALLBACK_GPDRAIL_ENABLE
                             );
#ifdef SL_CATALOG_TIMING_TEST_PRESENT
  sli_timing_end_tick = timer->CNT;
#endif // SL_CATALOG_TIMING_TEST_PRESENT
  APP_ASSERT((NULL != sl_rail_handle_gpdrail),
             "RAIL_Init failed, return value: NULL");

#if SL_RAIL_UTIL_INIT_DATA_FORMATS_GPDRAIL_ENABLE
  RAIL_DataConfig_t data_config = {
    .txSource = SL_RAIL_UTIL_INIT_DATA_FORMAT_GPDRAIL_TX_SOURCE,
    .rxSource = SL_RAIL_UTIL_INIT_DATA_FORMAT_GPDRAIL_RX_SOURCE,
    .txMethod = SL_RAIL_UTIL_INIT_DATA_FORMAT_GPDRAIL_TX_MODE,
    .rxMethod = SL_RAIL_UTIL_INIT_DATA_FORMAT_GPDRAIL_RX_MODE,
  };
  status = RAIL_ConfigData(sl_rail_handle_gpdrail, &data_config);
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "RAIL_ConfigData failed, return value: %d",
             status);
#endif // SL_RAIL_UTIL_INIT_DATA_FORMATS_GPDRAIL_ENABLE

#if SL_RAIL_UTIL_INIT_PROTOCOLS_GPDRAIL_ENABLE
  const RAIL_ChannelConfig_t *channel_config = NULL;
  if (SL_RAIL_UTIL_INIT_PROTOCOL_GPDRAIL_DEFAULT
      == SL_RAIL_UTIL_PROTOCOL_PROPRIETARY) {
#if SL_RAIL_UTIL_INIT_RADIO_CONFIG_SUPPORT_GPDRAIL_ENABLE
    channel_config = channelConfigs[SL_RAIL_UTIL_INIT_PROTOCOL_PROPRIETARY_GPDRAIL_INDEX];
#else // !SL_RAIL_UTIL_INIT_RADIO_CONFIG_SUPPORT_GPDRAIL_ENABLE
    APP_ASSERT(false,
               "SL_RAIL_UTIL_INIT_RADIO_CONFIG_SUPPORT_GPDRAIL_ENABLE must be true when (SL_RAIL_UTIL_INIT_PROTOCOL_GPDRAIL_DEFAULT == SL_RAIL_UTIL_PROTOCOL_PROPRIETARY)");
#endif // SL_RAIL_UTIL_INIT_RADIO_CONFIG_SUPPORT_GPDRAIL_ENABLE
  }
  (void) RAIL_ConfigChannels(sl_rail_handle_gpdrail,
                             channel_config,
                             &sli_rail_util_on_channel_config_change);
  status = sl_rail_util_protocol_config(sl_rail_handle_gpdrail,
                                        SL_RAIL_UTIL_INIT_PROTOCOL_GPDRAIL_DEFAULT);
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "sl_rail_util_protocol_config failed, return value: %d",
             status);
#endif // SL_RAIL_UTIL_INIT_PROTOCOLS_GPDRAIL_ENABLE

#if SL_RAIL_UTIL_INIT_CALIBRATIONS_GPDRAIL_ENABLE
  status = RAIL_ConfigCal(sl_rail_handle_gpdrail,
                          0U
                          | (SL_RAIL_UTIL_INIT_CALIBRATION_TEMPERATURE_NOTIFY_GPDRAIL_ENABLE
                             ? RAIL_CAL_TEMP : 0U)
                          | (SL_RAIL_UTIL_INIT_CALIBRATION_ONETIME_NOTIFY_GPDRAIL_ENABLE
                             ? RAIL_CAL_ONETIME : 0U));
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "RAIL_ConfigCal failed, return value: %d",
             status);
#endif // SL_RAIL_UTIL_INIT_CALIBRATIONS_GPDRAIL_ENABLE

#if SL_RAIL_UTIL_INIT_EVENTS_GPDRAIL_ENABLE
  status = RAIL_ConfigEvents(sl_rail_handle_gpdrail,
                             RAIL_EVENTS_ALL,
                             SL_RAIL_UTIL_INIT_EVENT_GPDRAIL_MASK);
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "RAIL_ConfigEvents failed, return value: %d",
             status);
#endif // SL_RAIL_UTIL_INIT_EVENTS_GPDRAIL_ENABLE

#if SL_RAIL_UTIL_INIT_TRANSITIONS_GPDRAIL_ENABLE
  RAIL_StateTransitions_t tx_transitions = {
    .success = SL_RAIL_UTIL_INIT_TRANSITION_GPDRAIL_TX_SUCCESS,
    .error = SL_RAIL_UTIL_INIT_TRANSITION_GPDRAIL_TX_ERROR
  };
  RAIL_StateTransitions_t rx_transitions = {
    .success = SL_RAIL_UTIL_INIT_TRANSITION_GPDRAIL_RX_SUCCESS,
    .error = SL_RAIL_UTIL_INIT_TRANSITION_GPDRAIL_RX_ERROR
  };
  status = RAIL_SetTxTransitions(sl_rail_handle_gpdrail,
                                 &tx_transitions);
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "RAIL_SetTxTransitions failed, return value: %d",
             status);
  status = RAIL_SetRxTransitions(sl_rail_handle_gpdrail,
                                 &rx_transitions);
  APP_ASSERT((RAIL_STATUS_NO_ERROR == status),
             "RAIL_SetRxTransitions failed, return value: %d",
             status);
#endif // SL_RAIL_UTIL_INIT_TRANSITIONS_GPDRAIL_ENABLE
#else // !SL_RAIL_UTIL_INIT_GPDRAIL_ENABLE
  // Eliminate compiler warnings.
  (void) sl_rail_handle_gpdrail;
#endif // SL_RAIL_UTIL_INIT_GPDRAIL_ENABLE
}

RAIL_Handle_t sl_rail_util_get_handle(sl_rail_util_handle_type_t handle)
{
  RAIL_Handle_t *sl_rail_handle_array[] = {
    &sl_rail_handle_gpdrail,
  };
